...
listSelfDeleting initialElems newElems externalSelections mkChild = mdo
  rec children     :: Dynamic t (Map Int (b, Event t DeselectMe, Event t DeleteMe, Event t SelectMe)) <- listWithKeyShallowDiff initialElems dElems mkChild
      ...
      delElems <- fmap ((fmap.fmap) (const Nothing) . switchPromptlyDyn) $ mapDyn (mergeMap . fmap (\(_,_,c,_) -> c)) children
      unselectElems  <- fmap ((fmap.fmap) (const Nothing) . switchPromptlyDyn) $ mapDyn (mergeMap . fmap (\(_,b,_,_) -> b)) children
      ...
      selection <- holdDyn Nothing $ leftmost [selNew, externalSelections, unselectSelected, selectElems ]
